                         ...
                         <span 
                                ...
                                {settingsDropdownOpen && (
                                    <ul className="absolute top-0 left-full ml-2 bg-white shadow-lg rounded-md w-32 text-md">
                                        <li>
                                        <span onClick={toggleModal} className="block px-4 py-2 text-gray-800 hover:bg-gray-200 cursor-pointer">Edit Profile</span>
                                        </li>
                                        <hr className="border-t border-gray-300 my-1" />
                                        <li>
                                            <Link to={'/delete-account'} className="block px-4 py-2 text-gray-800 hover:bg-gray-200">Delete Account</Link>
                                        </li>
                                    </ul>
                                )}
                         </li>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="relative bg-[#f9f9f9] border-2 border-[#dd3333] shadow-md rounded-lg w-1/3 p-6">
                        ...
                        <div className="mb-4">
                            ...
                            <input type="text" placeholder="Enter Your New Name" value={newName} onChange={(e) => setNewName(e.target.value)} className="w-full p-2 border border-[#dd3333] shadow-md rounded-md" />
                        </div>
                        <div className="mb-4">
                            <label className="block mb-1 text-sm text-[#dd3333] font-semibold">Email</label>
                            <input type="email" placeholder="Enter Your New E-mail" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="w-full p-2 border border-[#dd3333] shadow-md rounded-md" />
                        </div>
                        <div className="flex flex-col">
                            <button className="w-full py-3 bg-[#dd3333] text-white rounded-md font-semibold hover:bg-[#ff4444]" onClick={handleSaveChanges}>Save Changes</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isDeleteModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                        <div className="relative bg-[#f9f9f9] border-2 border-[#dd3333] shadow-md rounded-lg w-1/3 p-6">
                            ...
                            <p className="text-center text-gray-600 mb-6">Are you sure you want to delete your account? This action cannot be undone.</p>
                            <div className="flex justify-around">
                                {/* Delete Button */}
                                <button onClick={handleDeleteAccount} className="px-4 py-2 bg-[#dd3333] text-white rounded-md font-semibold hover:bg-[#ff4444]">Delete</button>
                                {/* Cancel Button */}
                                <button onClick={toggleDeleteModal} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md font-semibold hover:bg-gray-300">Cancel</button>
                            </div>
                        </div>
                    </div>
                )}
